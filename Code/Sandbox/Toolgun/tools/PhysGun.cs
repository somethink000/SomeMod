...
//[Library( "physgun", Title = "Rope", Description = "Join two things together with a rope", Group = "construction" )]
public class PhysGun : BaseTool
{

	[Property] public float MinTargetDistance { get; set; } = 0.0f;
	[Property] public float MaxTargetDistance { get; set; } = 100000000.0f;
	...
	public override void OnPrimaryPressed()
	{
		...
		var eyeDir = Owner.Camera.Transform.Rotation.Forward;
		var eyeRot = Owner.EyeAngles;

		if ( Grabbing && HeldBody.IsValid() )
		{
		...
		var tr = Scene.Trace.Ray( ViewRay, MaxTargetDistance )
	...
	public override void OnSeccondTap()
	{
	...
	private void TryUnfreezeAll( Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		bool unfrozen = false;
			...
			var body = rootEnt.Components.Get<Rigidbody>().PhysicsBody;
	...
	private void TryStartGrab( Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		//
		if ( body.BodyType == PhysicsBodyType.Keyframed && tr.Component is not PlayerBase )
			return;
		...
		GrabbedObject.Tags.Add( $"{GrabbedTag}{Owner.SteamID}" );
	...
	private void GrabEnd()
	{
		...
		if ( GrabbedObject.IsValid() )
		{
			...
			GrabbedObject.Tags.Remove( $"{GrabbedTag}{Owner.SteamID}" );
		}
	...
	private void GrabMove( Vector3 startPos, Vector3 dir, Rotation rot, bool snapAngles )
	{
		...
		if ( GrabbedObject.Root.Components.TryGet<PlayerBase>( out var player ) )
		{
